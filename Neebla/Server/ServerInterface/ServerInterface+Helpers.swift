...
extension ServerInterface {
    func syncHelper(result: SyncResult) throws {
        ...
        case .noIndex(let sharingGroups):
            try AlbumModel.upsertSharingGroups(db: Services.session.db, sharingGroups: sharingGroups, updateDownloadIndicators: true)
        ...
        case .index(sharingGroupUUID: let sharingGroupUUID, index: let index):
            ...
            try AlbumModel.upsertSharingGroups(db: Services.session.db, sharingGroups: sharingGroups, updateDownloadIndicators: false)
