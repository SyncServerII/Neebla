...
protocol DiscussionVCDelegate {
    // In each case the ServerFileModel references the discussion file.
    func discussionVC(_ vc: DiscussionVC, resetUnreadCount:ServerFileModel)
    func discussionVC(_ vc: DiscussionVC, changedDiscussion:ServerFileModel)
    func discussionVC(_ vc: DiscussionVC, discussion:ServerFileModel, refreshWithCompletion: (()->())?)
...
class DiscussionVC: MessagesViewController {
    ...
    private var discussion: ServerFileModel!
    ...
    private func loadDiscussion() -> Bool {
        ...
        self.fixedObjectsURL = url
        do {
            commentFile = try CommentFile(with: fixedObjectsURL)
        } catch let error {
            logger.error("Problem loading messages: \(error)")
            return false
        }
        
        guard commentFile != nil else {
            logger.error("Problem loading messages: Could not load from file!")
            return false
        }
    ...
    @discardableResult
    func setup(discussion: ServerFileModel, delegate:DiscussionVCDelegate?, usingNavigationController: Bool = true, closeHandler:(()->())? = nil) -> Bool {
...
//        parentVC.present(nav, animated: true, completion: nil)
        ...
        self.delegate?.discussionVC(self, resetUnreadCount: discussion)
...
extension DiscussionVC: MessagesDataSource {
    ...
    private func messageForItem(at indexPath: IndexPath) -> MessageType {
        ...
        let dict = commentFile![indexPath.section] as! [String: String]
...
extension DiscussionVC: InputBarAccessoryViewDelegate {
        ...
        for component in inputBar.inputTextView.components {
            if let text = component as? String {
                ...
                guard let fixedObject = message.toDictionary() else {
                    logger.error("Failed creating message!")
                    return
                }
                
                do {
                    
                    try commentFile.add(newRecord: fixedObject)
                    try commentFile.save(toFile: fixedObjectsURL)
                } catch (let error) {
                    logger.error("Failed adding message: \(error)")
                }
                
                messagesCollectionView.insertSections([commentFile.count - 1])
