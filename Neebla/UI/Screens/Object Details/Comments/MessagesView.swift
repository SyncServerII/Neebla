...
extension MessagesView.Coordinator: MessagesDataSource {
    func currentSender() -> SenderType {
        #warning("FIX ME")
        return Sender(senderId: "1", displayName: "Chris")
    }
...
extension MessagesView.Coordinator: InputBarAccessoryViewDelegate {
    func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
        ...
        let sender = Sender(senderId: "1", displayName: "Chris")
        let message = MockMessage(sender: sender, messageId: "1", sentDate: Date(), kind: .text("Foo"))
